...
7. このキーワードで上位表示するために本文で必ずカバーすべき「関連サブトピック・共起語」10個 (検索者が同時に知りたいこと。例:「費用」なら相場/内訳/安くする方法/追加費用/相見積り)`;
}
...
# AIO要件 (AI検索に引用されるための必須ルール)
20. 鮮度表示・対象者表示の直後に「この記事の要点」として箇条書き3行を置く。各行は60字以内の完結した断言文にする (AIが回答にそのまま抜粋できる形)
21. 各H2の直下の最初の段落は、その見出しの問いへの「40〜60字の直接回答」から始める (強調スニペット・AI引用の獲得型)。詳細説明はその後に続ける
22. FAQの回答は40〜60字の断言形に統一する (前置きなしで結論から)
23. 構成案の「関連サブトピック・共起語」を本文で自然に網羅する (羅列ではなく、該当セクションの文脈に組み込む)
