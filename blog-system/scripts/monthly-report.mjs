...
12. 優先施策マトリクス — 全提案施策を「インパクト×工数」で高効果×低工数から順に番号付け
13. 来月の数値目標とアクションプラン — 目標KPI (根拠つき) と実行順タスクリスト (担当・所要目安つき)

# 可視化ルール
- すべての主要数値に前月比を付ける。改善=▲、悪化=▼、横ばい=→
- 推移・比率は必ず表+バー (█ の繰り返し) で表現し、文章だけで説明しない
- 表は6列以内。1つの表に詰め込みすぎない
- **表の直前には必ず空行を1行置く** (直前が太字ラベルでも空行必須。Markdownの表が正しく描画されなくなるため)
